...
\end{aligned}\end{equation*}\todo{maybe just mention that it is easy to see that they are adjoint instead of writing all that}
...
% "sources": http://www.wisdom.weizmann.ac.il/~nadler/Publications/diffusion_map_MMS.pdf, http://howellkb.uah.edu/DEtext/Part7_BVProbs/SL.pdf p32f
...
Note that $c_n$ is a generalized fourier series of the initial distribution $p(\cdot, 0)$, i.e.\todo{not in this case(?) since $p(x,0) = \delta(x_0-x)$ and the Dirac-Delta function is not a smooth function - any way to rescue this ansatz?} $p(\cdot, 0) = \sum_k c_k \varphi_n$ on $[a,b]$ because the set of orthonormal eigenfunctions forms a basis. It is chosen such that \eqref{eq:eigenvalOfTn} holds true for $t=0$. As a result
...
Remember that, as the adjoint\footnote{under the standard inner product} of $\mathcal{L}$, $\mathcal{L}^*$ shares the eigenvalues $\lambda_n$. It is easy to see that $\psi_0(x)=1$ is the eigenfunction corresponding to the eigenvalue $\lambda_0 = 0$.

%Analogously\todo{einmal langsam durchrechnen} to the previous part it can be shown that

...
Note that $a_n$ is a generalized fourier series of the initial distribution $p(\cdot, 0)$, i.e.\todo{not in this case(?) since $p(x,0) = \delta(x_0-x)$ and the Dirac-Delta function is not a smooth function - any way to rescue this ansatz?} $p(\cdot, 0) = \sum_k a_k \varphi_n$ on $[a,b]$ because the set of orthonormal eigenfunctions forms a basis. It is chosen such that \eqref{eq:eigenvalOfTn} holds true for $t=0$. As a result
